...
function generateAllTypes() {
  console.log('🔄 Generating TypeScript types from C# Response classes...');
  console.log(`📂 Source: ${RESPONSES_DIR}`);
  console.log(`📝 Output: ${OUTPUT_FILE}`);
  
  if (!fs.existsSync(RESPONSES_DIR)) {
    console.error(`❌ Error: Responses directory not found at ${RESPONSES_DIR}`);
    process.exit(1);
  }
  ...
  if (files.length === 0) {
    console.warn('⚠️  No C# files found in Responses directory');
    return;
  }
  ...
  files.forEach(file => {
    ...
    if (classInfo) {
      ...
      if (tsInterface) {
        ...
        console.log(`  │  ✓ Generated ${classInfo.className}`);
      }
    }
  });
...
`;
  ...
  console.log(`✅ Successfully generated ${interfaces.length} TypeScript interface(s)`);
